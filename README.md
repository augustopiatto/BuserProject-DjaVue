...
```bash
dk bash  # starts bash inside "famass" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and famass.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `famass` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The famass container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv famass -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database famass owned by a user famass.

On your remote machine, create a file ~/famass.env:

```
DJANGO_DB_PASSWORD=<famass's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=famass
DJANGO_DB_USER=famass
DJANGO_DEBUG=0
```
...
```
server {
    server_name  famass.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/famass/static;
...
(Replace "famass.example.com" with your production domain)
